...
from tqdm import tqdm#!/usr/bin/env python3
import csv
import os
import time
from datetime import datetime
...
def get_with_retries(session, url, retries=3, backoff=2):
    """GET with HTTPError retry + exponential back-off."""
...
def parse_detail(session, href):
    """Fetch & parse a single listing detail page."""
    url = href if href.startswith("http") else BASE_URL + href
    resp = get_with_retries(session, url)
    soup = BeautifulSoup(resp.text, "html.parser")
    data = {}

    # extract tag-item fields
...
def scrape_page(session, page_num, total_bar):
    """Fetch listing URLs on one page, then parse each detail in parallel."""
    ...
    with ThreadPoolExecutor(max_workers=MAX_DETAIL_WORKERS) as executor:
        futures = {
            executor.submit(parse_detail, session, href): href
            for href in hrefs
        }
...
def main():
    ...
    with open(OUTPUT_FILE, "w", newline="", encoding="utf-8") as f:
        ...
        # process in chunks to reset session periodically
